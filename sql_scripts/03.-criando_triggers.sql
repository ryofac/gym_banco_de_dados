...
-- CREATE FUNCTION PROIBIR_CLIENTE_NAO_MATRICULADO()
-- RETURNS TRIGGER AS $$
-- BEGIN
-- 	 IF NOT EXISTS(SELECT * FROM CLIENTE C JOIN MATRICULA M ON C.ID_CLIENTE = M.ID_CLIENTE WHERE M.ID_CLIENTE = NEW.ID_CLIENTE) THEN
...
-- CREATE TRIGGER TRG_PROIBIR_CLIENTE_NAO_MATRICULADO
-- BEFORE INSERT OR UPDATE ON PLANO_TREINO
-- FOR EACH ROW
-- EXECUTE FUNCTION PROIBIR_CLIENTE_NAO_MATRICULADO();