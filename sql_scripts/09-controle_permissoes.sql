...
GRANT SELECT, INSERT, UPDATE, DELETE ON cliente TO funcionario;

GRANT SELECT, INSERT, UPDATE, DELETE ON matricula TO funcionario;

GRANT SELECT, INSERT, UPDATE, DELETE ON venda TO funcionario;
...
GRANT EXECUTE ON FUNCTION registrar_produto_na_compra (INT, INT, INT) TO
funcionario;

GRANT SELECT, INSERT, UPDATE, DELETE ON informacoes_matriculas TO funcionario;

GRANT SELECT, INSERT, UPDATE, DELETE ON informacoes_vendas TO funcionario;
...
GRANT SELECT, INSERT, UPDATE ON cliente TO instrutor;

GRANT SELECT, INSERT, UPDATE ON plano_treino TO instrutor;

-- Conceder permissões de execução apenas nas funções que manipulam plano de treino e cliente
GRANT EXECUTE ON FUNCTION criar_plano_de_treino (INT, INT, VARCHAR, VARCHAR) TO
instrutor;

GRANT EXECUTE ON FUNCTION adicionar_exercicio_no_treino (INT, INT, INT, INT, INT
) TO instrutor;

GRANT EXECUTE ON FUNCTION visualizar_plano_treino (INT) TO instrutor;

GRANT SELECT, INSERT, UPDATE, DELETE ON informacoes_matriculas TO instrutor;

GRANT SELECT, INSERT, UPDATE, DELETE ON informacoes_vendas TO instrutor;
...
GRANT usage ON SCHEMA PUBLIC TO gerente, funcionario, instrutor, estagiario;

-- Criando de usuários:
-- Criando os usuários que serão atribuídos aos roles
CREATE USER usuario_gerente WITH password 'senha123';

CREATE USER usuario_funcionario WITH password 'senha123';

CREATE USER usuario_instrutor WITH password 'senha123';

CREATE USER usuario_estagiario WITH password 'senha123';
